    ...
    <main class="main users chart-page" id="skip-target">
      <div class="container">
        <div style="display:flex;flex-direction:column">
          <div style="font-size:40px;text-align:center"><p>CHI TIẾT TIỀN HÀNG</p></div>
        </div>
        <form  action="/user/statistics/totalVNDetailBetween2Days" method="post">
          ...
          <div style="margin:3px;width:100%">
            <table>
              <tr >
                ...
                <th style="text-align:center">Tiền hàng (CNY)</th>
                <th style="text-align:center">Tiền hàng (VNĐ)</th>
              </tr>
              <c:forEach items="${totalVNDetailUser}" var="t" >
                <c:set var="totalVN" value="${totalVN + t.totalVN}"></c:set>
                <c:set var="totalCN" value="${totalCN + t.totalCN}"></c:set>
              <tr >
                <td><fmt:formatDate value = "${t.receivedDate}" type ="date"/></td>
                <td><a href="/user/order/detail/${t.orderNo}" target="_blank">${t.orderNo}</a></td>
                <td>${t.orderStatus}</td>
              ...
              <tr >
                ...
                <th ></th>
                <th >Tổng </th>
                <th style="text-align:center"><fmt:formatNumber value = "${totalCN}" type = "number"/></th>
                <th style="text-align:center"><fmt:formatNumber value = "${totalVN}" type = "number"/></th>
              </tr>
            </table>
          </div>
        </div>
          ...
          <div style="display:flex;flex-direction:row;justify-content: space-between;margin:10px">
            <div>
              <li class="page-item <c:if test="${page == 1}">disabled</c:if>">
                <a class="page-link" href="/user/statistics/totalVNDetail?page=${page - 1}&perpage=${perpage}" tabindex="-1" aria-disabled="true">Lùi lại</a>
              </li></div>
            <c:forEach begin="1" end="${total}" step="1" var="num">
              <div>  <li class="page-item <c:if test="${page == num}">active</c:if> ">
                <a class="page-link" href="/user/statistics/totalVNDetail?page=${num}&perpage=${perpage}">${num}</a>
              </li></div>
            </c:forEach>
            <div>
              <li class="page-item <c:if test="${page == total}">disabled</c:if>">
                <a class="page-link" href="/user/statistics/totalVNDetail?page=${page + 1}&perpage=${perpage}">Tiếp theo</a>
              </li></div>
          </div>
        </ul>
      </nav>
